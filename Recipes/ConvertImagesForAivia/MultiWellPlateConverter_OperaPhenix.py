...
file_pattern_info = file_patterns_info['r01c01f01p01-ch1sk1fk1fl1.tiff']    # TODO: integrate in UI when choosing files
image_extension = '.tiff'

# Image options         TODO: integrate in UI
flip_img_X = False
flip_img_Y = True

# Max projection option         TODO: integrate in UI
do_max_proj = True
...
def run(params):
    ...
    global file_pattern_info, image_extension, flip_img_X, flip_img_Y
                    ...
                    if ch == 1:
                        ...
                        if image_tag.getElementsByTagName('PositionZ'):
                            ...
                            if current_z == 1:
                                ...
                                xy_rel_positions[fov_name] = [
                                    float(image_tag.getElementsByTagName('PositionX')[0].firstChild.data) * 1E6,
                    ...
                    for c in range(1, image_dims[0] + 1):
                        for t in range(1, image_dims[1] + 1):
                            for z in range(1, image_dims[2] + 1):
                                ...
                                f_path_search = ''.join(updated_file_parts)

                                if not os.path.exists(f_path_search):  # File Not Found!
                                    print(f'Expected image was not found for these dimensions: T={t}, Z={z}, C={c}')
                                    stack_list.append('')
                                else:
                                    f = f_path_search
                                    f_count += 1
